...
try {
	...
	*/
	...
	mobile = /android|iphone|ipod|ipad|tablet|smartphone|ios/gmi.test(UA = navigator.userAgent), falseReg =
		/^(false|null|""|''|0|off|no|[]|{}|``|undefined|NaN|)$/gmi;
	Object.defineProperties(AUTO, {
		STRICT: { value: 1, writable: false, configurable: false }
		, HEAVY: { value: 2, writable: false, configurable: false }
		, LIGHT: { value: 4, writable: false, configurable: false }
		, NOCENTER: { value: 8, writable: false, configurable: false }
		, FILL: {
			value: 16
			, writable: false
			, configurable: false
		}
		, STROKE: { value: 32, writable: false, configurable: false }
		, FULL: {
			value: 64
			, writable: false
			, configurable: false
		}
		, UNLOCK: { value: 128, writable: false, configurable: false }
		, LOCK: { value: 256, writable: false, configurable: false }
	});
	...
	addEventListener("load", tick = function tick() {
		...
		ele("node")
			...
			.forEach(function(val) {
				...
				val.innerHTML = eval(val.getAttribute("data-func")
					.replace("this", "val"));
	...
	function ele(val, bl, comm, pr) {
		...
		if(par.match(/\[(?!\\(?!\\)).*?-(?!\\(?!\\)).*?\](?!\\(?!\\))/gmi) && !pr) {
			com = [];
			var ind = par.match(/\[.*?\]/mi)[0].replace(/(\[|\])(?!\\(?!\\))/gmi, "")
				.split("-");
			par = par.replace(/\[.*?\]/mi, "");
		...
		if(par.match(/^#(?!\\(?!\\))/gmi) && !pr) {
			...
			if(par.match(/\[(?!\\(?!\\))\d+?(?=\](?!\\(?!\\)))/mi)) {
				com = bl.getElementsByClassName(par.replace(/(^\.|\[(?!\\(?!\\)).+?\])(?!\\(?!\\))/gmi, ""))[Number(par.match(
					/\[(?!\\(?!\\))\d+?(?=\](?!\\(?!\\)))/mi)[0].replace(/^\[(?!\\(?!\\))/mi, ""))];
			}
			else {
				bo = false;
			...
			if(par.match(/\[(?!\\(?!\\))\d+?(?=\](?!\\(?!\\)))/mi)) {
				com = bl.getElementsByTagName(par.replace(/(^ |\[(?!\\(?!\\)).+?\])(?!\\(?!\\))/gmi, ""))[Number(par.match(
					/\[(?!\\(?!\\))\d+?(?=\](?!\\(?!\\)))/mi)[0].replace(/^\[(?!\\(?!\\))/mi, ""))];
			}
			else {
				bo = false;
			...
			if(par.match(/\[(?!\\(?!\\))\d+?(?=\](?!\\(?!\\)))/gmi)) {
				com = bl.getElementsByName(par.replace(/^@(?!\\(?!\\))/mi, ""))[Number(par.match(/\[(?!\\(?!\\))\d+?(?=\](?!\\(?!\\)))/mi)[0].replace(
					/^\[(?!\\(?!\\))/mi, ""))];
			}
			else {
				bo = false;
		...
		if((!bo) && comm) {
			com = cmp(com)
				.filter(function(n) {
					return eval(comm.replace(/@(?!\\(?!\\))/gmi, "n")
						.replace(/\\(?!\\(?!\\))/gmi, ""));
				});
		}
		else if(!bo) {
	...
	function mas(pat, arr, cond) {
		...
		for(var mat = 0; mat < arr.length; mat++) {
			if(eval(cond.replace(/@(?!\\(?!\\))/gmi, "arr[mat]"))) {
				ret[mat] = eval(pat.replace(/@(?!\\(?!\\))/gmi, "arr[mat]")
					.replace(/\\(?!\\(?!\\))/gmi, ""));
			}
		}
		return ret;
	...
	function cor(tr, d) {
		...
		if(ele("#corcor") && !tr) {
			ele()
				.removeChild(ele("#corcor"));
			return;
		}
		if(tr && !ele("#corcor")) {
			tr = ele()
				.appendChild(doc.createElement("coord"));
			...
			var T = setInterval(function() {
				if(ele("#corcor")) {
					ele("#corcor")
						.innerHTML = crd;
					ele("#corcor")
						.style.top = crd[1] + 5 + "px";
					ele("#corcor")
						.style.left = crd[0] + 5 + "px";
				}
				else {
					clearInterval(T);
				}
	...
	function src(elem) {
		...
		if(!ele("#srcsrc")) {
			doc.write("<textarea id='srcsrc' style='position:fixed;top:0%;left:0%;width:100%;height:100%;resize:none;' noresize>" + elem.outerHTML +
				"</textarea>");
			doc.close();
			dbg(true);
			ele("#dbgdbg")
	...
	function xtr(elem, typ) {
		...
		if(!typ) {
			...
			mas("doc.write([@,'<br>-----------<br>'].join(''))", mas("@.replace(/(src|href)=(\"|\')?/gmi,'')", elem.outerHTML.match(
				/(src|href)=(\"|\')?.+?(?=(\"|\'|( *?)?\>))/gmi)));
			doc.close();
			dbg(true);
			ele("#dbgdbg")
	...
	function mul(elem, comm) {
		...
		for(var stp = 0; stp < prefix.length; stp++) {
			eval("elem.style." + prefix[stp] + comm.charAt(0)
				.toUpperCase() + comm.replace(/^./gmi, "")
				.replace(/@(?!\\(?!\\))/gmi, "-" + prefix[stp] + "-")
				.replace(/\\(?!\\)/gmi, ""));
		}
	} //mul
	...
	function exp(elem, milli, init, comm) {
		...
		if(!elem.getAttribute("data-exp")) {
			var size = [/px/gm.test(elem.style.width) ? elem.style.width.match(/^.+?(?=px)/gm)[0] : elem.offsetWidth, /px/gm.test(elem.style.height) ? elem.style
                .height.match(/^.+?(?=px)/gm)[0] : elem.offsetHeight];
	...
	function tra(elem, milli, init, comm) {
		...
		if(!elem.getAttribute("data-tra")) {
			...
			var pos = [/px/gm.test(elem.style.left) ? elem.style.left.match(/^.+?(?=px)/gm)[0] : elem.offsetLeft, /px/gm.test(elem.style.top) ? elem.style.top.match(
				/^.+?(?=px)/gm)[0] : elem.offsetTop];
		...
		if(/heavy/gmi.test(auto.toString()) || (Auto & AUTO.HEAVY) == AUTO.HEAVY) {
			rnd = function(frm, to, rd) {
				if(frm === undefined) {
					...
					frm = tmp[0];
					to = tmp[1];
					return !rd || [frm, to].every(function(val) { return !/\./gmi.test(val.toString()) }) ? Math.round(Math.random() * (to - frm) + frm) :
		...
		if(!Array.prototype.datatype) {
			Array.prototype.__defineGetter__("datatype", function() {
				...
				for(var stp = 0; stp < this.length; stp++) {
					if((typeof this[stp] == "number" && !type) || (typeof this[stp] == "string" && type < 2) || ((this[stp] instanceof Array) && type <
							3) || ((this[stp] instanceof Object) && type < 4)) {
						type++
					}
				}
		...
		if(dY !== undefined && r === undefined && s === undefined) {
			...
			if(x1 != x2 && X1 != X2) {
				...
				if(l != L) {
					var x = (V - v) / (l - L);
					var y = l * x + v;
					if(x >= Math.min(x1, x2) && x <= Math.max(x1, x2) && x >= Math.min(X1, X2) && x <= Math.max(X1, X2) && y >= Math.min(y1, y2) && y <= Math.max(
	...
	function rep(cnt, com, ini) {
		...
		for(var stp = (ini ? ini : 0); stp < cnt + (ini ? ini : 0); stp++) {
			if(typeof com == "string") {
				val.push(eval(com.replace(/@(?!\\(?!\\))/gmi, stp)
					.replace(/\\(?!\\(?!\\))/gmi, "")));
			}
			else {
				val.push(com(stp));
	...
	function deg(dg, rd) {
		if(typeof dg != "string" && !rd) {
			...
			return(Number(dg.toString()
				.replace("π", Math.PI)) / Math.PI) * 180;
		}
	} //deg
	...
	function crt(e, el) {
		...
		return(el || ele())
			.appendChild(document.createElement(e));
	} //crt
...
[String, Array, Number].forEach(function(val) {
	...
	Array.prototype.values = Object.prototype.values = function() {
		return Object.keys(this)
			.filter(function(val) { return val != "Values" ? true : false })
			.map((function(val) {
					return this[val];
				})
				.bind(this));
	...
	function par(fun, num, nam, cod) {
		...
		return eval(fun + "=function " + fun + "(" + arr.join(",") + "," + nam + "," + nam + "0){" + nam + "=[" + arr.join(",") + "];" + nam + "0=['" + fun +
			"'," + num + "];" + (typeof cod == "string" ? cod : "return (" + cod + ")()") + "}");
	} //par
	//^deprecated
	...
	Object.prototype.last = function(off) {
		return this[Object.keys(this)[Object.keys(this)
			.length - 1 - (off ? off : 0)]];
	};
	...
	try {
		requestAnimation = requestFrame = animationFrame = requestAnimationFrame = webkitRequestAnimationFrame || mozRequestAnimationFrame ||
			oRequestAnimationFrame || khtmlRequestAnimationFrame || msRequestAnimationFrame || ieRequestAnimationFrame || requestAnimationFrame || setTimeout;
		if(!frame) {
		...
		Fullscreen = requestFullScreen = webkitRequestFullScreen || mozRequestFullScreen || oRequestFullScreen || khtmlRequestFullScreen || msRequestFullScreen ||
			ieRequestFullScreen || requestFullScreen || empty;
	}
	catch(e) {}
	if(!escape) {
	...
	String.prototype.splice = function(str, end) {
		return this.split("")
			.splice(str, end ? end : (this.length - str))
			.join("");
	};
	...
	Matrix = function Matrix(array) {
		...
		Object.defineProperty(this, "_row", {
			...
			, set: function(val) {
				this.m = this.values.copy()
					.cmp(1, this.values.length / val);
				this.row = this.m.length;
				this.column = this.m[0].length
			}
		});
		Object.defineProperty(this, "_column", {
			...
			, set: function(val) {
				this.m = this.values.copy()
					.cmp(1, this.column = val);
				this.row = this.m.length
			}
		});
		Object.defineProperty(this, "_values", {
			get: function() { return cmp(this.m) }
			, set: function(val) {
				this.values = val;
				this.m = this.values.copy()
					.cmp(1, this.column);
	...
	function D(x, y, z) {
		...
		this.t = function(c, fl) {
			...
			var x = deg(this.c[3])
				...
				, Y = this.y - this.c[1]
				, Z = this.z - this.c[2];
			var nc = [Math.cos(y) * (Math.sin(z) * Y + Math.cos(z) * X) - Math.sin(y) * Z, Math.sin(x) * (Math.cos(y) * Z + Math.sin(y) * (Math.sin(z) * Y +
				Math.cos(z) * X)) + Math.cos(x) * (Math.cos(z) * Y - Math.sin(z) * X), Math.cos(x) * (Math.cos(y) * Z + Math.sin(y) * (Math.sin(z) *
				Y + Math.cos(z) * X)) - Math.sin(x) * (Math.cos(z) * Y - Math.sin(z) * X)];
			this.X = nc[0];
			this.Y = nc[1];
			this.Z = nc[2];
			if(fl) {
