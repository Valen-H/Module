...
try {
	...
	canvas = new Object();
	if(typeof AUTO == "undefined") AUTO = new Object();
	...
	var crd = [0, 0]
		, ps = false
		, dr = [null, null, null, null, null, false]
		, zo = [null, null, null, null, null, false]
		, tr = [null, null, null, false];
	...
	function ark(x, y, r, a, b, c) {
		...
		if(auto) {
			stk(auto.toString()
				.match(/fill/gmi) || (Auto & AUTO.FILL) == AUTO.FILL ? true : false);
			return mov(x, y);
		}
		return d = [x + rx, y];
	} //ark
	...
	function grd(x, y, r, x1, y1, r1, pt) {
		...
		for(var stp = 0; stp < (pt ? pt : y1)
			.split("/")
			.length; stp++) {
			gr.addColorStop(Number((pt ? pt : y1)
					.split("/")[stp].split(":")[0]) / 100, (pt ? pt : y1)
				.split("/")[stp].split(":")[1]);
		}
		if(auto) {
	...
	function grb(cn) {
		if(typeof module === "undefined") {
			...
			if(typeof cn == "string") {
				...
				cn = ele("canvas[0]") ? ele("canvas[0]") : (ele()
	...
	function edt(dt, ed, pre, post) {
		...
		for(var stp = 0; stp < dt.data.length; stp += 4) {
			var Red = red = dt.data[stp]
				, Green = green = dt.data[stp + 1]
				, Blue = blue = dt.data[stp + 2]
				, Alpha = alpha = dt.data[stp + 3]
				, width = dt.width
				, height = dt.height;
			var grey = (red + blue + green) / 3
				, clr = red > blue ? (red > green ? "red" : "green") : (blue > green ? "blue" : "green");
			...
			var yellow = (red + green) / 2 > blue + 10 && red > blue && green > blue
				, purple = (red + blue) / 2 > green + 10 && red > green && blue > green
				, cyan = (green + blue) / 2 > red + 10 && green > red && blue > red;
			if(yellow) {
				clr = "yellow";
			}
			else if(purple) {
			...
			var Yellow = (red + green) / 2 - blue
				, Purple = (red + blue) / 2 - green
				, Cyan = (blue + green) / 2 - red;
			if(typeof ed != "string") {
				ed();
			}
			else {
	...
	ImageData.prototype.edt = function(ed, pre, post) {
		return this.data = edt(this, ed, pre, post)
			.data;
	};
	...
	function Det(Can) {
		(mobile ? ["touchstart", "touchmove"] : ["mousedown", "click", "mousemove", "wheel"])
		.forEach(function(val) {
			(Can || can)
			.addEventListener(val, function(e) {
				if(Can) e.preventDefault()
				if(e.touches) {
					if(e.touches.length) {
						drgs = [(e.touches.last()
							.clientX - Off[0]) * un[0] - trns[0], (e.touches.last()
							.clientY - Off[1]) * un[1] - trns[1]];
					}
				}
				else {
